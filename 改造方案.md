...
> **实施同步（2026-08-07）**：第四阶段已完成并准备提交。站点已建立 Docusaurus 3.10.2、TypeScript、Mermaid、显式 sidebar 和 GitHub Pages 构建/部署工作流；原静态首页已归档到 `legacy/index.html`，活动首页改为 `src/pages/index.tsx`。Overview、Concepts、Append/Recovery、Read、Storage Profile、Object/BK WAL、Generation、Materialization、Pulsar facade/cursor、Native Kafka/KoP 边界、Trim/Retention/GC、Failure/Recovery、Backpressure/Observability、Security、Correctness authorities 以及项目状态和文档基线页面已落地，PDF 覆盖表位于 `migration/pdf-content-map.yml`。当前网页文档基线为 `nereus main@c820391d`（2026-08-07 核验），PDF 输入快照仍标记为 `nereus main@894fc4e`（2026-08-04 生成）；覆盖表中的 pending 项不能被视为已迁移。下一阶段继续实现 Module Boundaries、End-to-end Examples、Non-goals、Glossary、State Machines、Cheatsheet 和 Source Reading Map，并在每个稳定增量后同步更新本段与覆盖表。
