...
> **实施同步（2026-08-07）**：第二阶段已完成并准备提交。站点已建立 Docusaurus 3.10.2、TypeScript、Mermaid、显式 sidebar 和 GitHub Pages 构建/部署工作流；原静态首页已归档到 `legacy/index.html`，活动首页改为 `src/pages/index.tsx`。Overview、Concepts、Append/Recovery、Read、Storage Profile、Object/BK WAL、Generation 以及项目状态和文档基线页面已落地，PDF 覆盖表位于 `migration/pdf-content-map.yml`。当前网页文档基线为 `nereus main@c820391d`（2026-08-07 核验），PDF 输入快照仍标记为 `nereus main@894fc4e`（2026-08-04 生成）；覆盖表中的 pending 项不能被视为已迁移。下一阶段继续实现 Pulsar/Native Kafka 接入、Materialization 以及 Operations/Reference，并在每个稳定增量后同步更新本段与覆盖表。
